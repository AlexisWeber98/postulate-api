...
describe("Postulation Service", () => {
  ...
  describe("postPostulationService", () => {
    it("should create a postulation if user exists", async () => {
      ...
      (Postulations.create as jest.Mock).mockResolvedValue({
        id: "1",
        company: "Test",
      });
    ...
    it("should throw NotFoundError if user does not exist", async () => {
      ...
      await expect(postPostulationService({} as any)).rejects.toThrow(
        NotFoundError
      );
    });
  });
  ...
  describe("getAllPostulationsService", () => {
    it("should return paginated postulations", async () => {
      (Postulations.findAndCountAll as jest.Mock).mockResolvedValue({
        count: 1,
        rows: [{ id: "1" }],
      });

      const result = await getAllPostulationsService(
        "1",
        {},
        { page: 1, limit: 10 }
      );
  ...
  describe("getPostulationByIdService", () => {
    ...
    it("should throw NotFoundError if postulation does not exist", async () => {
      ...
      await expect(getPostulationByIdService("1")).rejects.toThrow(
        NotFoundError
      );
    });
  });
  ...
  describe("updatePostulationService", () => {
    it("should update a postulation if it exists", async () => {
      ...
      expect(Postulations.findByPk).toHaveBeenCalledWith("1");
      expect(mockPostulation.update).toHaveBeenCalledWith({
        company: "Updated",
      });
    });
    ...
    it("should throw NotFoundError if postulation does not exist", async () => {
      ...
      await expect(updatePostulationService("1", {})).rejects.toThrow(
        NotFoundError
      );
    });
  });
  ...
  describe("deletePostulationService", () => {
    ...
    it("should throw NotFoundError if postulation does not exist", async () => {
      ...
      await expect(deletePostulationService("1")).rejects.toThrow(
        NotFoundError
      );
    });
  });
